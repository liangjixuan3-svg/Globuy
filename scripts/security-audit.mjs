...
export function auditEntries(entries, secrets = []) {
  ...
  for (const { path, content } of entries) {
    ...
    if (emails.some(e => !e.endsWith('@example.invalid') && e !== 'example@usc.edu' && !e.endsWith('@users.noreply.github.com'))) fail('non-demo email')
    if (/(?<![\d.])1[3-9]\d{9}(?![\d.])/.test(content)) fail('possible phone number')
