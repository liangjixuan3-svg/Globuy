...
test('公开演示保留原虚构商品及图片、用户、订单、聊天，不包含原密码', async () => {
  ...
  assert.deepEqual([...messages].sort((a,b) => a.id-b.id), seed.chat.filter(c => (c.fromUserId === 1 && c.toUserId === 2) || (c.fromUserId === 2 && c.toUserId === 1)).map(c => ({ ...c, text: c.text.replaceAll('demo-assets/', '/Globuy/demo-assets/') })))
...
test('原用户的多个地址能完整展示，并使用实际选中的地址下单', async () => {
  ...
  assert.deepEqual(rows, seed.address.filter(a => a.userId === seed.sysUser[0].id).sort((a,b) => b.id-a.id))
